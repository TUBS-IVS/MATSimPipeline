...
def is_whole_number(value):
    """Check if the value is a whole number as a string or integer."""
...
def test_enhance_travel_survey(setup_test_environment):
    ...
    # Load the trips file to compare row counts
    hh_df = pd.read_csv(paths['test_mid_trips_folder'])
    num_hhs = len(hh_df)
    num_output_rows = len(output_df)

    # Ensure the number of rows in the output is between 2 and 4 times the number of households
    assert num_hhs * 2 <= num_output_rows <= num_hhs * 5, (
    ...
    grouped = output_df.groupby('PERSON_ID')
    ...
    for person_id, group in grouped:
        if len(group) > 1:
            ...
            for i, unique_leg_id in enumerate(unique_leg_ids):
                last_part = int(unique_leg_id.split('_')[-1])
                assert last_part == int(non_unique_leg_ids[i]), (
            ...
            # Check that each person has a main activity
