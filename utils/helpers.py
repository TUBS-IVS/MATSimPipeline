...
class Helpers:
    def __init__(self, project_root="", output_folder="", config=None, stats_tracker=None, logger=None):
        """
        ...
        self.logger = logger if logger is not None else logging.getLogger("helpers")

    def open_text_file(self, file_path, mode):
        """
    ...
    def modify_text_file(self, input_file, output_file, replace, replace_with):
        """
        Replace text in a text file.
        Also works for gzipped files.
        """
        self.logger.info(f"Replacing '{replace}' with '{replace_with}' in {input_file}...")
        with self.open_text_file(input_file, 'rt') as f:
            file_content = f.read()

        modified_content = file_content.replace(replace, replace_with)

        with self.open_text_file(output_file, 'wt') as f:
            f.write(modified_content)
        self.logger.info(f"Wrote modified file to {output_file}.")

    def create_leg_ids(self, legs_file):
        """
        ...
        """
        self.logger.info(f"Creating unique leg ids in {legs_file}...")
        if s.LEG_ID_COL in legs_file.columns:
            self.logger.info(f"Legs file already has unique leg ids, skipping.")
            return
    ...
    def seconds_from_datetime(self, datetime):
        """
    ...
    def compress_to_gz(self, input_file, delete_original=True):
        self.logger.info(f"Compressing {input_file} to .gz...")
        with open(input_file, 'rb') as f_in:
            with gzip.open(f"{input_file}.gz", 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
        if delete_original:
            os.remove(input_file)
        self.logger.info(f"Compressed to {input_file}.gz.")

    def find_outer_boundary(self, gdf, method='convex_hull'):
    ...
    def distribute_by_weights(self, data_to_distribute: pd.DataFrame, weighted_points_in_cells: pd.DataFrame,
                              external_id_column,
        ...
        """
        self.logger.info("Starting distribution by weights...")

        if not data_to_distribute[external_id_column].isin(weighted_points_in_cells[external_id_column]).all():
            if cut_missing_ids:
                self.logger.info(f"Not all geography IDs in the population dataframe are in the weights dataframe. "
        ...
        self.logger.info(f"Computed ID counts for {len(id_counts)} unique IDs.")
        ...
        def distribute_rows(group):
            ...
            if total_count == 0 or pd.isna(total_count):
                self.logger.debug(f"Geography ID {group[external_id_column].iloc[0]} is not in the given dataframe, "
        ...
        self.logger.info(f"Generated expanded weights DataFrame with {len(expanded_weights_df)} rows.")
        ...
        data_to_distribute = pd.merge(data_to_distribute, expanded_weights_df,
                                      on=[external_id_column, '_processing_seq'],
                                      how='left').drop(columns='_processing_seq')

        self.logger.info("Completed distribution by weights.")
    ...
    def calculate_condition_likelihoods(self, df, filter_col, target_col) -> dict:
        """
        ...
        self.logger.info(f"Calculated likelihoods for {len(likelihoods)} unique values in {filter_col}.")
        self.logger.info(f"{likelihoods}")
        return likelihoods

    def calculate_value_frequencies_df(self, df, filter_col, target_col) -> pd.DataFrame:
        """
        ...
        self.logger.info(f"Calculated frequencies for {len(frequencies_df)} unique values in {filter_col}.")
        self.logger.info(f"{frequencies_df}")

        return frequencies_df

    def summarize_slack_factors(self, slack_df):
        """
        ...
        :return:
        """
        self.logger.info(f"Summarizing slack factors for {len(slack_df)} rows...")
        slack_df = slack_df[(slack_df['slack_factor'] > 1) & (slack_df['slack_factor'] < 50)]
        self.logger.info(f"Dropped outliers and false positives, {len(slack_df)} rows remaining.")
        ...
        self.logger.info(f"Summarized slack factors for {len(summary_df)} unique activity combinations.")
        return summary_df

    def calculate_travel_time_matrix(self, cells_gdf, speed, detour_factor=1.3):
        """
        ...
        for i, from_point in enumerate(centers):
            self.logger.debug(f"Calculating travel times for cell {i} of {len(centers)}...")
    ...
    def ensure_crs(self, gdf: gpd.GeoDataFrame, crs: str = 'EPSG:25832') -> gpd.GeoDataFrame:
        """
        ...
        """
        if gdf.crs != crs:
            self.logger.info(f"Converting GeoDataFrame from {gdf.crs} to {crs}...")
            return gdf.to_crs(crs)
        self.logger.info(f"GeoDataFrame already in {crs}, skipping conversion.")
        return gdf

    def sigmoid(self, x, beta, delta_T):
        """
    ...
    def check_distance(self, leg_to_find, leg_to_compare):
    ...
    def check_time(self, leg_to_find, leg_to_compare):
    ...
    def check_mode(self, leg_to_find, leg_to_compare):
        """
    ...
    def check_activity(self, leg_to_find, leg_to_compare):
        ...
        if (type_to_find == type_to_compare or
            ...
            self.logger.debug("Activity Type Undefined or Null (which usually means person has no legs).")
            return False
    ...
    def add_from_cell_fast(self, person):
        """
    ...
    def find_nan_chains(self, df, column_name):
        """
    ...
    def assign_points(self, df, shapefile_path, df_cell_name_column, gdf_cell_name_column, point_column_name):
        """
        ...
        """
        self.logger.info("Assigning points based on cells...")
        # Read the shapefile
        gdf = gpd.read_file(shapefile_path)
        self.logger.info(f"Loaded shapefile with {len(gdf)} cells.")
        ...
        for index, row in df.iterrows():
            ...
            if pd.isna(row[point_column_name]):
                if cell_name in cell_geometry_map:
                    ...
                    self.logger.debug(f"Assigning point for cell: {cell_name}")
                    point = random_point_in_cell(cell_geometry_map[cell_name])
                    df.at[index, point_column_name] = point

            else:
                self.logger.debug(f"Skipping assignment for cell: {cell_name}")

        return df

    def translate_column(self, df: pd.DataFrame, source_col: str, new_col: str, value_type: str, from_key: str,
                         to_key: str) -> pd.DataFrame:
        """
        Generalized method for translating columns using value maps from the configuration.
        """

        # Retrieve the appropriate value map from config
        value_map_path = f"value_maps.{value_type}"
        value_map = self.config.get(value_map_path)

        if not isinstance(value_map, dict):
            self.logger.error(f"Expected dictionary at config path '{value_map_path}', but got {type(value_map)}.")
            raise ValueError(f"Invalid value map format for '{value_type}'.")

        # Construct translation dictionary
        translation_dict = {v.get(from_key): v.get(to_key) for k, v in value_map.items() if v.get(from_key) is not None}

        self.logger.info(f"Translating column '{source_col}' to '{new_col}' using value type '{value_type}'.")
        ...
        if len(missing_values) > 0:
            self.stats_tracker.log(f"missing_translations.{value_type}.{source_col}", list(missing_values))
            self.logger.warning(
                f"Missing translations for {len(missing_values)} unique values in column '{source_col}': {missing_values}")

        return df

    def generate_unique_household_id(self, df):
        col_name = s.UNIQUE_HH_ID_COL
        self.logger.info(f"Generating unique household IDs...")
    ...
    def generate_unique_person_id(self, df):
    ...
    def generate_unique_leg_id(self, df):
    ...
    def build_estimation_tree(self, distances: List[float]) -> List[List[List[float]]]:  # Tree level, Leg, Lengths
        """
        ...
        """
        self.logger.debug(f"Building estimation tree for {len(distances)} legs.")
        ...
        while len(distances) > 1:
            ...
            for i in range(0, len(distances) - 1, 2):
                combined_list: List[float] = self.estimate_length_with_slack(distances[i], distances[i + 1])
    ...
    def estimate_length_with_slack(self, length1, length2, slack_factor=2, min_slack_lower=0.2, min_slack_upper=0.2) -> \
            List[
                float]:
    ...
    def get_files(self, path: str, get_all: bool = False) -> Union[str, List[str]]:
    ...
    def euclidean_distance(self, start: np.ndarray, end: np.ndarray) -> float:
        """Compute the Euclidean distance between two points."""
        return np.linalg.norm(end - start)

    def get_min_max_distance(self, arr):
    ...
    def spread_distances(self, distance1, distance2, iteration=0, first_step=20, base=1.5):
    ...
    def get_abs_distance_deviations(self, candidate_coordinates, location, wanted_distance):
    ...
    def get_main_activity_leg(self, person_legs):  #: UnSegmentedPlan old format
    ...
    def filter_feasible_data(self, data):
        ...
        for person_id, trips in data.items():
            ...
            for segments in trips:
                ...
                direct_distance = self.euclidean_distance(first_location, last_location)

                # Check feasibility of this segment group
                if not self.check_feasibility(distances, direct_distance):
    ...
    def check_feasibility(self, distances, direct_distance, consider_total_distance=True):
        """
        @Author: Hoerl, Sebastian
        """
        return self.calculate_feasibility(distances, direct_distance, consider_total_distance) == 0.0

    def calculate_feasibility(self, distances, direct_distance, consider_total_distance=True):
        """
        @Author: Hoerl, Sebastian
        """
        # Calculate the feasibility of any chain
    ...
    def downsample_population(self, df, sample_percentage):
        """
...
class SlackFactors:
    ...
    def __init__(self, slack_factors_csv_path: str):
        ...
        self.logger.info(f"Loaded slack factors from {slack_factors_csv_path}.")
    ...
    def get_slack_factor(self, activity_from: str, activity_via: str, activity_to: str) -> float:
        ...
        if not slack_factor_row.empty:
            ...
            self.logger.debug(f"No slack factor found for activities: {activity_from}, {activity_via}, {activity_to}. "
    ...
    def calculate_expected_time_with_slack(self, time_from_start_to_via: float, time_from_via_to_end: float,
        ...
        if expected_time < time_diff or expected_time > time_sum:  # this makes the trip impossible. We find a reasonable alternative:
            self.logger.debug(f"Expected time is infeasible. Returning time difference plus half of the smaller time.")
            expected_time = time_diff + (min(time_from_start_to_via, time_from_via_to_end) / 2)
        return expected_time

    def calculate_expected_distance_with_slack(self, distance_from_start_to_via: float,
                                               distance_from_via_to_end: float,
    ...
    def get_all_estimated_times_with_slack(self, leg_chain, level=0):
        ...
        if len_times_col == 0:
            self.logger.warning(f"Received empty DataFrame, returning unchanged.")
            return leg_chain, level
        elif len_times_col == 1:
            self.logger.warning(f"Received single leg, returning unchanged.")
            return leg_chain, level
        elif len_times_col == 2:
            self.logger.debug(f"Two legs remain to estimate, solving last level.")
            return self.solve_level(leg_chain, level), level + 1
        # Recursive case
        else:
            self.logger.debug(f"More than two legs remain to estimate, solving level {level}.")
    ...
    def get_all_adjusted_times_with_slack(self, leg_chain, real_total_time):
        ...
        if len(highest_lvl_leg_index) != 1:
            self.logger.error(f"Expected 1 leg in highest level, found {len(highest_lvl_leg_index)}.")
        ...
        if lower_bound <= real_total_time <= upper_bound:
            self.logger.debug(f"Real total time is within bounds of highest level.")
            return df, highest_level
        else:
            self.logger.debug(f"Real total time is outside bounds of highest level, adjusting...")
                    ...
                    if len(group) == 1:
                        ...
                        if higher_leg_value < higher_leg_lower_bound:  # We have strongly overshot because the REAL higher value is lower than the lower bound
                            ...
                            if leg1_value > leg2_value:
                                L_bounds1 = abs(
                                    df.at[
                                        group['original_index'].iloc[0], f'level_{level}_lower_bound'] - leg1_value)
                                L_bounds2 = abs(
                                    df.at[
                                        group['original_index'].iloc[1], f'level_{level}_upper_bound'] - leg2_value)
                                ...
                                L_bounds1 = abs(
                                    df.at[
                                        group['original_index'].iloc[0], f'level_{level}_upper_bound'] - leg1_value)
                                L_bounds2 = abs(
                                    df.at[
                                        group['original_index'].iloc[1], f'level_{level}_lower_bound'] - leg2_value)
    ...
    def solve_level(self, leg_chain, level):
        ...
        if level == 0:
            ...
            if leg_chain[times_col].notna().sum() != len(leg_chain):
                self.logger.warning(f"Found NaN values in {times_col}, may produce incorrect results.")
        else:
            times_col = f"level_{level}"  # Here we expect some NaN values
...
class TTMatrices:
    ...
    def __init__(self, car_tt_matrices_csv_paths: List[str], pt_tt_matrices_csv_paths: List[str],
        ...
        for mode, csv_paths in zip(['car', 'pt'], [car_tt_matrices_csv_paths, pt_tt_matrices_csv_paths]):
            for hour, path in enumerate(csv_paths):
                try:
                    ...
                    self.logger.error(f"Error reading {mode} matrix for hour {hour}: {e}")
                    raise e
        ...
        try:
            ...
            self.logger.error(f"Error reading bike/walk matrices: {e}")
            raise e
        ...
        for mode, matrices in self.tt_matrices.items():
            if mode in ['bike', 'walk']:
                if len(matrices) != 1:
                    self.logger.warning(f"Expected 1 {mode} matrix, found {len(matrices)}")
            else:
                if len(matrices) != 24:
                    self.logger.warning(f"Expected 24 {mode} matrices, found {len(matrices)}")

        self.logger.info(f"Loaded travel time matrices for {len(self.tt_matrices['car'])} hours.")
    ...
    def get_tt_matrix(self, mode: str, hour: int = None):
        ...
        if mode not in ['car', 'pt', 'bike', 'walk']:
            self.logger.error(f"Invalid mode: {mode}. Choices are 'car', 'pt', 'bike', 'walk'.")
            return self.tt_matrices['walk']

        if mode in ['car', 'pt']:
            if hour is None:
                self.logger.error(f"Hour must be specified for mode {mode}. Returning walk matrix.")
                return self.tt_matrices["walk"]
    ...
    def get_weighted_tt_matrix_two_modes(self, mode1, weight1, mode2, weight2, hour=None):
        ...
        weighted_tt_matrix['VALUE'] = tt_matrix1['VALUE'].multiply(weight1).add(
            tt_matrix2['VALUE'].multiply(weight2))
...
class Capacities:
    ...
    def __init__(self, capa_cells_shp_path: str = None, capa_points_shp_path: str = None,
        ...
        self.logger.info("Initializing capacities...")
        ...
        # self.logger.info(f"Created capacity_gdf for {len(self.capa_points_gdf)} points.")
        ...
        self.logger.info("Initialized capacities.")

    def translate_and_split_potentials(self, translation_dict=None):
        self.logger.info("Translating capacities...")
        ...
        self.logger.info("Translated capacities.")

    def round_capacities(self):
        """
        Round the capacities while preserving the sum.
        """
        self.logger.info("Rounding capacities to integers while preserving the sum...")
        for col in self.capa_csv_df.columns:
            ...
            while diff > 0:
                ...
                diff -= 1
        self.logger.info("Rounded capacities.")
    ...
    def get_capacity(self, activity_type, cell_name):
        ...
        try:
            ...
            self.logger.debug(f"Activity type {activity_type} was not converted.")
        ...
        try:
            ...
            self.logger.debug(f"Invalid activity type for capacity: {activity_type}")
            return 0
    ...
    def decrement_capacity(self, activity_type, cell_name):
        ...
        if activity_type not in self.capa_csv_df.columns:
            self.logger.debug(f"Invalid activity type for capacity: {activity_type}. Not decrementing.")
            return
        ...
        if not filtered_df.empty:
            ...
            self.logger.error(f"No capacity found for cell: {cell_name}. Not decrementing.")
    ...
    def normalize_capacities(self, total_values):
        ...
        """
        self.logger.info("Normalizing capacities...")
        ...
        self.logger.info("Normalized capacities.")
